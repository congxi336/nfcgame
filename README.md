...
一套「安卓手机碰 NFC 卡 → 读 UID → 查服务器自定义信息」的完整系统。含 **Node.js + Express + SQLite 服务器** 与 **Kotlin 安卓 App**，界面采用暗色荧光风格。

> ⚠️ 本系统仅读取 NFC 卡片的 UID（唯一标识符），不读取、不解析、不存储卡内任何数据（余额、交易记录等）。
...
- **隐私提示**：首次进入弹窗告知仅读取 UID、不读取卡内任何数据。
...
本项目仅实现 NFC 卡片 UID（唯一标识符）的读取，以及「UID → 自定义信息」的映射查询。项目**不包含、也不提供**以下任何能力：

- 读取、解析、解密卡内数据（余额、交易记录、密钥等）；
- 写入、复制、克隆、模拟或伪造任何实体卡片；
- 门禁、支付、票务、身份认证等场景的身份核验。

### 数据与隐私

- 服务器仅存储最小字段（`uid`、`title`、`content`、`image_url`），不采集、不存储持卡人的姓名、联系方式等任何个人身份信息。
- App 首次启动会展示隐私提示，如实告知「仅读取 UID、不读取卡内数据」的技术事实。

### 使用限制

使用本项目的个人或组织，应确保其使用行为符合所在地法律法规。**严禁**将本项目用于绕过门禁、盗刷、仿冒身份、伪造卡片，或任何侵害他人合法权益、危害公共安全的用途。

### 免责声明

本项目以「按现状（AS IS）」提供，作者不对以下情形承担责任：

- 使用者对项目的任何非法或不当使用及其造成的后果；
- 因部署、配置、网络环境、第三方服务等因素导致的任何损失；
- 服务器或 App 运行中出现的任何故障、数据丢失或安全事件。

本项目仅供学习、研究及合规的技术演示用途。
